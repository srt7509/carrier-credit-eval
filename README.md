# 延长物流信用评价系统（车辆信用评价 Demo）

基于 Flask + Vue 3 的车辆信用评价系统。以承运商所属车辆为评价对象，支持信用画像、双模型评分、业务联动配置、风险预警、区块链存证等功能。
...
```bash
# 从本地打包并上传部署（首次需要 --reset-db 重建数据库）
./deploy.sh <服务器IP> root --reset-db

# 后续更新（保留数据）
./deploy.sh <服务器IP> root
```
...
| 模块 | 路由 | 说明 |
|------|------|------|
| 车辆管理 | `/` | 运输品类×信用等级组合筛选，承运商维度扩展筛选 |
| 信用画像 | `/profile/:id` | 五维雷达图、评分趋势、事件时间线 |
