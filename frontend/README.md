...
本项目是团队参加2026年中国国际大学生创新大赛（产业赛道）的作品。基于 Vue 3 + Element Plus 的承运商信用评价系统，展示信用画像、模型监控、业务联动配置、风险预警、区块链存证等功能。
...
### 1. 承运商列表（CarrierList）
- 多条件组合筛选（信用等级、运输品类、风险标签、评分区间）
- 多字段排序
- 点击跳转信用画像

### 2. 信用画像（CreditProfile）
- 六维雷达图（企业资质、履约能力、服务质量、行为合规、经营信用、生态协同）
- 平台均值对比
- 近12月评分趋势折线图
- 扣分/加分事件时间线
- 评分构成解释
...
│   │   └── global.css   # 全局样式
│   ├── views/
│   │   ├── CarrierList.vue      # 承运商列表
│   │   ├── CreditProfile.vue    # 信用画像
